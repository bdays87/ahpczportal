<div>
    ...
    <x-card title="Accounts Payable Invoices" separator class="mt-5 border-2 border-gray-200">
        <x-slot:menu>
    ...
    {{-- Create / Edit Modal --}}
    <x-modal wire:model="modal" title="{{ $id ? 'Edit Invoice' : 'New AP Invoice' }}" box-class="max-w-3xl">
        <x-form wire:submit="save">
            <div class="grid grid-cols-2 gap-3">
                <x-choices-offline label="Supplier *" wire:model="supplier_id" :options="$suppliers" 
                    option-label="name" single searchable />
                <x-input label="Supplier Invoice #" wire:model="supplier_reference" />
                ...
                <x-choices-offline label="Expense Account *" wire:model="expense_account_id" :options="$expenseAccounts" 
                    option-label="name" single searchable />
                <x-choices-offline label="AP Account *" wire:model="ap_account_id" :options="$apAccounts" 
                    option-label="name" single searchable />
    ...
    {{-- View Invoice Modal --}}
    <x-modal wire:model="viewModal" title="AP Invoice — {{ $selectedInvoice?->invoice_number }}" box-class="max-w-2xl">
        @if($selectedInvoice)
            <div class="grid grid-cols-2 gap-3">
                <div><strong>Supplier:</strong> {{ $selectedInvoice->supplier?->name }}</div>
                <div><strong>Supplier Ref:</strong> {{ $selectedInvoice->supplier_reference }}</div>
